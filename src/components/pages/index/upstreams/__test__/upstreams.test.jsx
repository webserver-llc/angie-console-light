...
describe('<UpstreamsBox IndexPage />', () => {
	it('render()', () => {
		...
		// all / up row
		expect(indexBox.childAt(2).text()).toBe('Пиры: 3 / Активные: 3');
		...
		// all / up row
		expect(indexBox.childAt(2).text()).toBe('Пиры: 3 / Активные: 2');
		// failed row className
		expect(indexBox.childAt(3).prop('className')).toBe(styles.red);
		// failed row
		expect(indexBox.childAt(3).text()).toBe('Проблемные: 1');
