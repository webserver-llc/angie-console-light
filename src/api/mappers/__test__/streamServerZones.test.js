...
describe("Mapper - Stream Server Zones", () => {
  it("mapperStreamServerZones()", () => {
    ...
    expect(mapperStreamServerZones({}), "should be empty object").to.be.an("object")
      .that.is.empty;
    expect(
      mapperStreamServerZones(angieStreamServerZones),
      "should be correct format",
    ).to.be.deep.equal(nginxStreamServerZones);
  });
});
